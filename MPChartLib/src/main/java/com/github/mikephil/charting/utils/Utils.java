...
public abstract class Utils {
    ...
    /**
     * calculates the approximate height of a text, depending on a demo text and no of lines
     * avoid repeated calls (e.g. inside drawing methods)
     *
     * @param paint
     * @param demoText
     * @return
     */
    public static int calcTextHeight(Paint paint, String demoText, int lineCount) {

        Rect r = mCalcTextHeightRect;
        r.set(0,0,0,0);
        paint.getTextBounds(demoText, 0, demoText.length(), r);
        return r.height() * lineCount + 12 * (lineCount - 1); // 12 is spacing b/w two lines
    }
